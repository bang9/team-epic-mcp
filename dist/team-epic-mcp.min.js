...
- \uC5C5\uB370\uC774\uD2B8: ${N(e.last_updated)} by ${e.updated_by}`),t}function U(a){let t={progress:"\u{1F4CA}",blocker:"\u{1F6AB}",resolved:"\u2705",comment:"\u{1F4AC}",status_change:"\u{1F504}"}[a.update_type]||"\u{1F4DD}",r=a.platform?`[${a.platform}] `:"";return`${t} **${N(a.timestamp)}** ${r}- ${a.message} (by ${a.author})`}function N(a){return new Date(a).toLocaleDateString("ko-KR",{month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"})}var P=[{name:"list_epics",description:"Epic \uBAA9\uB85D\uC744 \uC870\uD68C\uD569\uB2C8\uB2E4. \uC0C1\uD0DC\uC640 \uB2F4\uB2F9\uC790\uB85C \uD544\uD130\uB9C1 \uAC00\uB2A5\uD569\uB2C8\uB2E4.",inputSchema:{type:"object",properties:{status:{type:"string",description:"Epic \uC0C1\uD0DC \uD544\uD130",enum:["backlog","kickoff","planning","development","code_review","testing","ready_to_release","released","done","on_hold"]},assignee:{type:"string",description:"\uB2F4\uB2F9\uC790 \uBA58\uC158 (\uC608: @Airen Kang)"}}},handler:(a,e)=>{let t=a.epics;e.status&&(t=t.filter(s=>s.current_status===e.status)),e.assignee&&(t=t.filter(s=>s.ios_assignee===e.assignee||s.android_assignee===e.assignee||s.js_assignee===e.assignee));let r=t.length>0?t.map(s=>{let i=a.epicStatuses.find(n=>n.epic_id===s.epic_id);return $(s,i)}).join(`
